...
//doh: source: https://stackoverflow.com/questions/5755998/appending-one-element-to-a-dynamic-array
  type
    TAppender<T> = class
...
//doh: source: https://stackoverflow.com/questions/5755998/appending-one-element-to-a-dynamic-array
// simple procedure to add elements to Tarray<T>
...
begin
    check_context(a, b);
    //    var r : Z3_ast := binop(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
    r := binop(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
    r := UnOp(a.ctx.ToZ3_Context, a.ToZ3_ast);
    //var r : Z3_ast := UnOp(a.ctx.ToZ3_Context, a.ToZ3_ast);
...
begin
  r := _fn(_arg.ctx.ToZ3_Context, _arg.ToZ3_ast);
...
begin
   ...
   r  := _fn( _arg1.ctx.ToZ3_Context, _arg1.ToZ3_ast, _arg2.ToZ3_ast);
   //var r : Z3_ast := _fn( _arg1.ctx.ToZ3_Context, _arg1.ToZ3_ast, _arg2.ToZ3_ast);
...
begin
    check_context(a, b);
    assert(a.is_bool and b.is_bool);
    //doh: add elements to args one by one
    args.Create;
    TAppender<Z3_ast>.Append(args, a.ToZ3_ast);
    TAppender<Z3_ast>.Append(args, b.ToZ3_ast);
//    args  := [ a.ToZ3_ast, b.ToZ3_ast ];


    r := Z3_mk_and(a.ctx.ToZ3_Context, 2, @args[0]);
    //var args : TArray<Z3_ast> := [ a.ToZ3_ast, b.ToZ3_ast ];
    //var r : Z3_ast := Z3_mk_and(a.ctx.ToZ3_Context, 2, @args[0]);
...
//    args  := [ a.ToZ3_ast, b.ToZ3_ast ];
   ...
   r := Z3_mk_or(a.ctx.ToZ3_Context, 2, @args[0]);
   //var args : TArray<Z3_ast> := [ a.ToZ3_ast, b.ToZ3_ast ];
   //var r : Z3_ast := Z3_mk_or(a.ctx.ToZ3_Context, 2, @args[0]);
...
begin
    _args := args.ToArray;
    r  := Z3_mk_or(args.ctx.ToZ3_Context, _args.size, _args.ptr);
    //var _args : TZ3Array<Z3_ast> := args.ToArray;
    //var r : Z3_ast := Z3_mk_or(args.ctx.ToZ3_Context, _args.size, _args.ptr);
...
begin
   _args := args.ToArray;
   r  := Z3_mk_and(args.ctx.ToZ3_Context, _args.size, _args.ptr);
   //var _args : TZ3Array<Z3_ast> := args.ToArray;
   //var r : Z3_ast := Z3_mk_and(args.ctx.ToZ3_Context, _args.size, _args.ptr);
...
begin
   ...
   check_context(c, e);
   assert(c.is_bool);
   //doh: duplicate lines, why?
   r := Z3_mk_ite(c.ctx.ToZ3_Context, c.ToZ3_ast, t.ToZ3_ast, e.ToZ3_ast);

   //doh: duplicates removed
   //r : Z3_ast := Z3_mk_ite(c.ctx.ToZ3_Context, c.ToZ3_ast, t.ToZ3_ast, e.ToZ3_ast);
   //var r : Z3_ast := Z3_mk_ite(c.ctx.ToZ3_Context, c.ToZ3_ast, t.ToZ3_ast, e.ToZ3_ast);
   //var r : Z3_ast := Z3_mk_ite(c.ctx.ToZ3_Context, c.ToZ3_ast, t.ToZ3_ast, e.ToZ3_ast);
...
begin
    assert(args.size > 0);
    ctx  := args.Items[0].ctx;
    _args  := args.ToArray;
    r  := Z3_mk_distinct(ctx.ToZ3_Context, _args.size, _args.ptr);
    //var ctx : TContext := args.Items[0].ctx;
    //var _args : TZ3Array<Z3_ast> := args.ToArray;
    //var r : Z3_ast := Z3_mk_distinct(ctx.ToZ3_Context, _args.size, _args.ptr);
...
var r : Z3_ast ;
    //_args  := [ a.ToZ3_ast, b.ToZ3_ast ];
    _args : TArray<Z3_ast>;// := [ a.ToZ3_ast, b.ToZ3_ast ];

begin
  check_context(a, b);
  //doh: not an inline declaration, only a declaration, removed
  //var r : Z3_ast ;
  ...
  begin
      ...
      //doh: add elements to args one by one
      TAppender<Z3_ast>.Append(_args, a.ToZ3_ast);
      TAppender<Z3_ast>.Append(_args, b.ToZ3_ast);
   //    args  := [ a.ToZ3_ast, b.ToZ3_ast ];
  ...
  begin
      ...
      r := Z3_mk_concat(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
var i : Integer;
    r : Z3_ast ;
    ctx : TContext;// := args.Items[0].ctx;
    _args : TZ3Array<Z3_ast>;// := args.ToArray;
...
begin
    check_context(a, b);
    r  := Z3_mk_eq(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
    //var r : Z3_ast := Z3_mk_eq(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
   ...
   r  := Z3_mk_distinct(a.ctx.ToZ3_Context, 2, @args[0]);
   //var args : TArray<Z3_ast> := [ a.ToZ3_ast, b.ToZ3_ast ];
   //var r : Z3_ast := Z3_mk_distinct(a.ctx.ToZ3_Context, 2, @args[0]);
...
begin
   ...
   ctx := args.Items[0].ctx;
   _args := args.ToArray;
   r := Z3_mk_add(ctx.ToZ3_Context, _args.size, _args.ptr);
   ctx.check_error;
   Result := TExpr.Create(ctx, r);
end;
...
begin
    ...
    ctx  := es.Items[0].ctx;
    _es  := es.ToArray;
    r  := Z3_mk_pble(ctx.ToZ3_Context, _es.size, _es.ptr, @coeffs, bound);
    ctx.check_error;
    Result := TExpr.Create(ctx, r);
end;
...
begin
    ...
    ctx  := es.Items[0].ctx;
    _es  := es.ToArray;
    r  := Z3_mk_pbge(ctx.ToZ3_Context, _es.size, _es.ptr, @coeffs, bound);
    //var ctx : TContext := es.Items[0].ctx;
    //var _es : TZ3Array<Z3_ast> := es.ToArray;
    //var    r: Z3_ast  := Z3_mk_pbge(ctx.ToZ3_Context, _es.size, _es.ptr, @coeffs, bound);
...
begin
   ...
   ctx := es.Items[0].ctx;
   _es  := es.ToArray;
   r  := Z3_mk_pbeq(ctx.ToZ3_Context, _es.size, _es.ptr, @coeffs, bound);
   //var ctx : TContext := es.Items[0].ctx;
   //var _es : TZ3Array<Z3_ast> := es.ToArray;
   //var    r: Z3_ast  := Z3_mk_pbeq(ctx.ToZ3_Context, _es.size, _es.ptr, @coeffs, bound);
...
begin
   ...
   ctx  := es.Items[0].ctx;
   _es  := es.ToArray;
   r    := Z3_mk_atmost(ctx.ToZ3_Context, _es.size, _es.ptr, bound);
   //var ctx : TContext := es.Items[0].ctx;
   //var _es : TZ3Array<Z3_ast> := es.ToArray;
   //var    r: Z3_ast  := Z3_mk_atmost(ctx.ToZ3_Context, _es.size, _es.ptr, bound);
...
begin
   ...
   ctx  := es.Items[0].ctx;
   _es  := es.ToArray;
   r  := Z3_mk_atleast(ctx.ToZ3_Context, _es.size, _es.ptr, bound);
   //var ctx : TContext := es.Items[0].ctx;
   //var _es : TZ3Array<Z3_ast> := es.ToArray;
   //var    r: Z3_ast  := Z3_mk_atleast(ctx.ToZ3_Context, _es.size, _es.ptr, bound);
...
begin
    check_context(a, b);
    r  := Z3_mk_bvand(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
    //var r : Z3_ast := Z3_mk_bvand(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
   check_context(a, b);
   r:= Z3_mk_bvxor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
   //var r : Z3_ast := Z3_mk_bvxor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
   check_context(a, b);
   r  := Z3_mk_bvor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
   //var r : Z3_ast := Z3_mk_bvor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
   check_context(a, b);
   r := Z3_mk_bvnand(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
   //var r : Z3_ast := Z3_mk_bvnand(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
  check_context(a, b);
  r  := Z3_mk_bvnor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_bvnor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
   check_context(a, b);
   r  := Z3_mk_bvxnor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
   //var r : Z3_ast := Z3_mk_bvxnor(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
  ...
  check_context(a, rm);
  assert(a.is_fpa);
  r := Z3_mk_fpa_sqrt(a.ctx.ToZ3_Context, rm.ToZ3_ast, a.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_fpa_sqrt(a.ctx.ToZ3_Context, rm.ToZ3_ast, a.ToZ3_ast);
...
begin
    r  := Z3_mk_bvnot(a.ctx.ToZ3_Context, a.ToZ3_app);
    //var r : Z3_ast := Z3_mk_bvnot(a.ctx.ToZ3_Context, a.ToZ3_app);
...
begin
  ...
  r  := Z3_mk_fpa_fma(a.ctx.ToZ3_Context, rm.ToZ3_ast, a.ToZ3_ast, b.ToZ3_ast, c.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_fpa_fma(a.ctx.ToZ3_Context, rm.ToZ3_ast, a.ToZ3_ast, b.ToZ3_ast, c.ToZ3_ast);
...
begin
  check_context(lo, hi);
  r  := Z3_mk_re_range(lo.ctx.ToZ3_Context, lo.ToZ3_ast, hi.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_re_range(lo.ctx.ToZ3_Context, lo.ToZ3_ast, hi.ToZ3_ast);
...
begin
    r  := Z3_mk_int2real(a.ctx.ToZ3_Context, a.ToZ3_ast);
    //var r : Z3_ast := Z3_mk_int2real(a.ctx.ToZ3_Context, a.ToZ3_ast);
...
begin
  check_context(a, i);
  r  := Z3_mk_select(a.ctx.ToZ3_Context, a.ToZ3_ast, i.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_select(a.ctx.ToZ3_Context, a.ToZ3_ast, i.ToZ3_ast);
...
begin
  check_context(a, i);
  idxs   := i.ToArray;
  r  := Z3_mk_select_n(a.ctx.ToZ3_Context, a.ToZ3_ast, idxs.size, idxs.ptr);
  //var idxs : TZ3Array<Z3_ast>  := i.ToArray;
  //var r : Z3_ast := Z3_mk_select_n(a.ctx.ToZ3_Context, a.ToZ3_ast, idxs.size, idxs.ptr);
...
begin
  check_context(a, i);
  check_context(a, v);
  r := Z3_mk_store(a.ctx.ToZ3_Context, a.ToZ3_ast, i.ToZ3_ast, v.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_store(a.ctx.ToZ3_Context, a.ToZ3_ast, i.ToZ3_ast, v.ToZ3_ast);
...
begin
  ...
  check_context(a, v);
  idxs  := i.ToArray;
  r  := Z3_mk_store_n(a.ctx.ToZ3_Context, a.ToZ3_ast, idxs.size, idxs.ptr, v.ToZ3_ast);
  //var idxs : TZ3Array<Z3_ast> := i.ToArray;
  //var r : Z3_ast := Z3_mk_store_n(a.ctx.ToZ3_Context, a.ToZ3_ast, idxs.size, idxs.ptr, v.ToZ3_ast);
...
begin
  r  := Z3_mk_as_array(f.ctx.ToZ3_Context, f.ToZ3_func_decl);
  //var r : Z3_ast := Z3_mk_as_array(f.ctx.ToZ3_Context, f.ToZ3_func_decl);
...
begin
  ...
  //doh: add elements to args one by one
  es.create;
  TAppender<Z3_ast>.Append(es, a);
  TAppender<Z3_ast>.Append(es, b);



  r   := Z3_mk_set_union(a.ctx.ToZ3_Context, 2, @es[0]);
  //var r : Z3_ast  := Z3_mk_set_union(a.ctx.ToZ3_Context, 2, @es[0]);
...
begin
  ...
  r  := Z3_mk_set_intersect(a.ctx.ToZ3_Context, 2, @es[0]);
  //var r : Z3_ast := Z3_mk_set_intersect(a.ctx.ToZ3_Context, 2, @es[0]);
...
begin
  r  := Z3_mk_seq_empty(s.ctx.ToZ3_Context, s.ToZ3_Sort);
  //var r : Z3_ast := Z3_mk_seq_empty(s.ctx.ToZ3_Context, s.ToZ3_Sort);
...
begin
  check_context(a, b);
  r  := Z3_mk_seq_suffix(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_seq_suffix(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
  check_context(a, b);
  r  := Z3_mk_seq_prefix(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_seq_prefix(a.ctx.ToZ3_Context, a.ToZ3_ast, b.ToZ3_ast);
...
begin
  ...
  r  := Z3_mk_seq_index(s.ctx.ToZ3_Context, s.ToZ3_ast, substr.ToZ3_ast, offset.ToZ3_ast);
  //var r : Z3_ast := Z3_mk_seq_index(s.ctx.ToZ3_Context, s.ToZ3_ast, substr.ToZ3_ast, offset.ToZ3_ast);
...
begin
  r  := Z3_mk_re_empty(s.ctx.ToZ3_Context, s.ToZ3_Sort);
  //var r : Z3_ast := Z3_mk_re_empty(s.ctx.ToZ3_Context, s.ToZ3_Sort);
...
begin
  r  := Z3_mk_re_full(s.ctx.ToZ3_Context, s.ToZ3_Sort);
  //var r : Z3_ast := Z3_mk_re_full(s.ctx.ToZ3_Context, s.ToZ3_Sort);
...
begin
  ...
  _args := args.ToArray;
  r := Z3_mk_re_intersect(ctx.ToZ3_Context, _args.size, _args.ptr);
  //var r : Z3_ast := Z3_mk_re_intersect(ctx.ToZ3_Context, _args.size, _args.ptr);
...
begin
  ...
  //vars  := [ x.ToZ3_app ];
  vars.Create;
  TAppender<Z3_ast>.Append(vars, x.ToZ3_app);


  r  := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 1, @vars[0], 0, 0, b.ToZ3_ast);
  //var vars : Tarray<Z3_app> := [ x.ToZ3_app ];
  //var r : Z3_ast := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 1, @vars[0], 0, 0, b.ToZ3_ast);
...
begin
  ...
  //doh:
  //vars := [ x1.ToZ3_app, x2.ToZ3_app];
  vars.create;
  TAppender<Z3_ast>.Append(vars, x1.ToZ3_app);
  TAppender<Z3_ast>.Append(vars, x2.ToZ3_app);

  r := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 2, @vars[0], 0, 0, b.ToZ3_ast);
  //var vars : Tarray<Z3_app> := [ x1.ToZ3_app, x2.ToZ3_app];
  //var r : Z3_ast := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 2, @vars[0], 0, 0, b.ToZ3_ast);
...
begin
  ...
  //doh
  //vars := [x1.ToZ3_app, x2.ToZ3_app, x3.ToZ3_app ];
  vars.create;
  TAppender<Z3_ast>.Append(vars, x1.ToZ3_app);
  TAppender<Z3_ast>.Append(vars, x2.ToZ3_app);
  TAppender<Z3_ast>.Append(vars, x3.ToZ3_app);

  r  := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 3, @vars[0], 0, 0, b.ToZ3_ast);
  //var vars : Tarray<Z3_app> := [x1.ToZ3_app, x2.ToZ3_app, x3.ToZ3_app ];
  //var r : Z3_ast := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 3, @vars[0], 0, 0, b.ToZ3_ast);
...
begin
  ...
  //doh
  //vars  := [x1.ToZ3_app, x2.ToZ3_app, x3.ToZ3_app, x4.ToZ3_app ];
  vars.create;
  TAppender<Z3_ast>.Append(vars, x1.ToZ3_app);
  TAppender<Z3_ast>.Append(vars, x2.ToZ3_app);
  TAppender<Z3_ast>.Append(vars, x3.ToZ3_app);
  TAppender<Z3_ast>.Append(vars, x4.ToZ3_app);

  r  := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 4, @vars[0], 0, 0, b.ToZ3_ast);
  //var vars : Tarray<Z3_app> := [x1.ToZ3_app, x2.ToZ3_app, x3.ToZ3_app, x4.ToZ3_app ];
  //var r : Z3_ast := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, 4, @vars[0], 0, 0, b.ToZ3_ast);
...
begin
  vars  := xs.ToArray;
  r  := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, vars.size, vars.ptr, 0, 0, b.ToZ3_ast);
  //var vars : TZ3Array<Z3_app> := xs.ToArray;
  //var r : Z3_ast := Z3_mk_forall_const(b.ctx.ToZ3_Context, 0, vars.size, vars.ptr, 0, 0, b.ToZ3_ast);
